...
goals<-read.csv("C:/Users/durda/Desktop/IE 582 HW/Datalar/goals.csv")
goals<-data.table(goals)

#Case 1
goals$time<-as.numeric(goals$time)

EPLmatches=matches[matches$league_id=='148',]
EPLmatches<-data.table(EPLmatches)
setkey(EPLmatches,match_id)
setkey(goals,match_id)
EPLmatches<-merge(EPLmatches,goals)
Case1<-EPLmatches[time>90]
EPLmatches<- EPLmatches[!(time>=90),]
EPLmatches<-EPLmatches[,-c('time','home_scorer','score','away_scorer')]
...
#Range 6
Range6Y<-testY[(HomeNormalizedProb-AwayNormalizedProb)>=0.6 & (HomeNormalizedProb-AwayNormalizedProb)<=0.9]
...
#Range 6
Range6E<-testE[(HomeNormalizedProb-AwayNormalizedProb)>=0.6 & (HomeNormalizedProb-AwayNormalizedProb)<=0.9,]
...
#Range 6
Range6S<-testS[(HomeNormalizedProb-AwayNormalizedProb)>=0.6 & (HomeNormalizedProb-AwayNormalizedProb)<=0.9,]
...
S<-c((R1S/length(Draw1S$match_id)),(R2S/length(Draw2S$match_id)),(R3S/length(Draw3$match_id)),(R4S/length(Draw4$match_id)),(R5S/length(Draw5S$match_id)),(R6S/length(Draw6S$match_id)))
...
>> By comparing last graphs of Task 2 and Task 3, there is some loss in number of games. This affected our coise of the best company and the intervals which gives better results. Since Betolimp gave 1/1 perfect recommendations for 3 intervals, i choose again Betolimp but in here from intervals 2 to 5 is better for this option
